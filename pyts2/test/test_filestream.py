from pyts2.filestream import FileStream
...
def test_read():
    ...
    for timestream in timestreams:
        ...
        #    else:
        #        assert inst.datetime in SMALL_TIMESTREAMS["expect_times"]
        #        assert inst.subsecond == 0
        #        assert inst.index is None


def test_gvlike():
    for i, file in enumerate(FileStream("testdata/timestreams/gvlike")):
...
def test_zipout(tmpdir):
    def check_output_ok(outpath):
        for i, file in enumerate(FileStream(outpath)):
            expect_inst = TSInstant(SMALL_TIMESTREAMS["expect_times"][i],
                                    subsecond=0, index=None)
            assert file.instant == expect_inst
    ...
    for level in outputs.keys():
        ...
        if level == "root":
            outpath += ".zip"
        out = FileStream(path=outpath, mode="w", format="tif",
                         bundle_level=level, name="output")
        for file in FileStream("testdata/timestreams/nested"):
            out.write(file)
        out.close()
